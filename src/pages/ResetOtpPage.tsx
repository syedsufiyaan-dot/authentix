...
export const ResetOtpPage: React.FC = () => {
  ...
  const handleVerify = async (codeOverride?: string) => {
    ...
    try {
      ...
      toast('Reset code verified. Please set your new password.', 'success');
  ...
  const handleResend = async () => {
    ...
    setErrorMessage('');
    try {
      const msg = await resendOtp(email, 'password_reset');
  ...
  return (
    <div className="min-h-screen bg-bg-darkest text-text-primary flex flex-col justify-between select-none">
      ...
      <main className="flex-1 flex items-center justify-center p-6 md:p-12">
        ...
        >
          <div className="space-y-2">
            ...
            <h2 className="text-2xl font-bold text-white tracking-tight">Enter Reset Code</h2>
          ...
          <div className="flex items-center justify-between text-xs font-mono text-text-muted">
            <span>
              {expirySeconds > 0 ? `Expires in ${formatSeconds(expirySeconds)}` : <span className="text-status-critical">Code expired</span>}
            </span>
            <button
              type="button"
      ...
      <footer className="px-6 py-4 text-center text-xs text-text-dim font-mono border-t border-bg-border/30">
        AuthentiX Verification Gateway
      </footer>
    </div>
  );
};
